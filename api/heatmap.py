...
BASE_URL = os.getenv("PROJECT_HOST") + ":" + os.getenv("PROJECT_PORT")
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        return {
            ...
            "download_url":f"{BASE_URL}/download/{filename_only}",
            "heatmap_base64": heatmap_base64
        }
...
async def multi_predict(files: list[UploadFile] = File(...)):
    results = []
    ...
    # 2. most confident class (by highest average confidence)
    best_class = max(
